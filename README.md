...
工作负载整体和TPC-C保持一致。除了原TPC-C配置，工具还支持设置以下参数：
- `skew`：bool值参数（默认false），表示是否启用数据倾斜等工作负载。
- `skew.seed`：整数参数（默认0），特殊工作负载的随机数种子。
- `skew.alphaData`: 浮点参数（默认0），数据分布不均衡的alpha值。0表示数据分布均匀，大于0表示数据分布不均匀，alpha值越小，数据分布越不均衡。
- `skew.alphaTxn`: 浮点参数（默认0），数据热点的alpha值。0表示数据访问分布均匀，大于0表示存在数据热点，alpha值越小，数据热点越集中。
- `skew.updateInterval`: 整数参数（默认0），数据热点更新间隔，单位为秒。0表示数据热点不发生动态变化。

b)故障场景配置

故障场景配置主要包括对故障类型、故障范围、故障参数和故障时间的配置。
